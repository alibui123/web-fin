...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ loading: false, success: true, error: false, message: data?.message || "Message successfully transmitted." });
      setFormData({ name: "", email: "", subject: "", message: "" });
      setTimeout(() => { setStatus((s) => ({ ...s, success: false, message: "" })); }, 5000);
    } catch (error: any) {
  ...
  const inputClass =
    "w-full bg-white/[0.02] border border-white/10 rounded-xl px-5 py-4 text-white focus:ring-0 focus:outline-none placeholder:text-white/20 transition-all duration-300 disabled:opacity-50 focus:border-finova-cyan/50 focus:bg-finova-cyan/[0.02] shadow-[inset_0_0_20px_rgba(255,255,255,0.01)]";
  ...
  return (
    <div className="bg-finova-midnight min-h-screen">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden" ref={heroRef}>
        ...
        <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-finova-magenta/10 blur-3xl opacity-30 rounded-full pointer-events-none will-change-transform" style={{ transform: "translateZ(0)" }} />
        {/* Grid */}
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          ...
          >
            <div className="flex items-center gap-2 px-4 py-2 rounded-full border border-finova-cyan/30 bg-finova-cyan/10 mb-8 shadow-[0_0_15px_rgba(14,165,233,0.15)]">
              <MessageSquare className="w-3.5 h-3.5 text-finova-cyan" />
            ...
            <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black mb-6 uppercase tracking-tighter text-white">
              <span className="block drop-shadow-[0_0_20px_rgba(255,255,255,0.15)]">Get In</span>
      ...
      {/* ════════════ MAIN CONTENT ════════════ */}
      <section className="relative pb-24 overflow-hidden" ref={formRef}>
        ...
        <div className="container mx-auto px-4 sm:px-6 md:px-12 relative z-10">
          <div className="grid lg:grid-cols-5 gap-8 max-w-6xl mx-auto items-start">
            ...
            >
              ...
              <div className="group relative rounded-3xl bg-white/[0.03] border border-white/10 p-8 overflow-hidden transition-all duration-500 hover:border-white/20 hover:shadow-[0_0_30px_rgba(147,51,234,0.15)] shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                <div className="absolute inset-0 bg-gradient-to-br from-finova-purple/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="absolute -right-10 -top-10 w-32 h-32 bg-finova-purple/10 blur-3xl rounded-full group-hover:bg-finova-purple/20 transition-colors duration-500" />
                <div className="relative z-10">
                  <div className="w-14 h-14 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center justify-center text-finova-purple mb-6 group-hover:scale-110 transition-transform duration-500 shadow-[0_0_15px_rgba(147,51,234,0.1)]">
                    <Clock className="h-6 w-6" />
                  </div>
                  <h3 className="text-xl font-black text-white mb-6 tracking-tight uppercase">Business Hours</h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-white/50 font-mono tracking-wide">Mon – Fri</span>
                      <span className="text-white/80 font-semibold">9:00 AM – 6:00 PM</span>
                    </div>
                    <div className="h-[1px] w-full bg-white/5" />
                    <div className="flex justify-between items-center">
                      <span className="text-white/50 font-mono tracking-wide">Sat – Sun</span>
              ...
              {/* Social / LinkedIn */}
              <div className="group relative rounded-3xl bg-white/[0.03] border border-white/10 p-6 overflow-hidden transition-all duration-500 hover:border-white/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                <div className="relative z-10 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-[#0A66C2]/10 border border-[#0A66C2]/20 flex items-center justify-center text-[#0A66C2] group-hover:scale-110 transition-transform duration-500">
                      <Linkedin className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm uppercase tracking-wide">LinkedIn</p>
                      <p className="text-white/40 text-xs font-mono">Follow for updates</p>
                    </div>
                  </div>
                  <a
                    ...
                    className="px-5 py-2.5 rounded-xl border border-white/10 text-xs font-bold uppercase tracking-widest text-white/60 hover:text-white hover:border-finova-cyan/50 hover:bg-finova-cyan/5 transition-all duration-300"
                  >
                    Connect
                  </a>
                </div>
            ...
            >
              ...
              <div className="absolute -top-20 -right-20 w-64 h-64 bg-finova-cyan/5 blur-3xl rounded-full pointer-events-none" />
              <div className="absolute -bottom-20 -left-20 w-64 h-64 bg-finova-magenta/5 blur-3xl rounded-full pointer-events-none" />

              <form onSubmit={handleSubmit} className="relative z-10 flex flex-col h-full justify-between gap-8">
                <div className="mb-2">
                  <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight mb-2">Send a Message</h2>
                  <p className="text-white/40 text-sm font-light">Fill in the details below and we&apos;ll get back to you promptly.</p>
                </div>
                ...
                <div className="space-y-6">
                  <div className="grid sm:grid-cols-2 gap-6">
                    ...
                    <div className="relative group">
                      <label htmlFor="cp-name" className="block text-xs font-bold tracking-widest uppercase text-white/50 mb-2 group-focus-within:text-finova-cyan transition-colors">Full Name</label>
                      <div className="relative">
                        <input id="cp-name" name="name" type="text" placeholder="John Doe" value={formData.name} onChange={handleChange} onFocus={() => setFocusedField("name")} onBlur={() => setFocusedField(null)} required disabled={status.loading} className={inputClass} />
                    ...
                    {/* Email */}
                    <div className="relative group">
                      <label htmlFor="cp-email" className="block text-xs font-bold tracking-widest uppercase text-white/50 mb-2 group-focus-within:text-finova-cyan transition-colors">Email Address</label>
                      <div className="relative">
                        <input id="cp-email" name="email" type="email" placeholder="john@example.com" value={formData.email} onChange={handleChange} onFocus={() => setFocusedField("email")} onBlur={() => setFocusedField(null)} required disabled={status.loading} className={inputClass} />
                  ...
                  {/* Subject */}
                  <div className="relative group">
                    <label htmlFor="cp-subject" className="block text-xs font-bold tracking-widest uppercase text-white/50 mb-2 group-focus-within:text-finova-cyan transition-colors">Subject</label>
                    <div className="relative">
                      <input id="cp-subject" name="subject" type="text" placeholder="How can we help?" value={formData.subject} onChange={handleChange} onFocus={() => setFocusedField("subject")} onBlur={() => setFocusedField(null)} required disabled={status.loading} className={inputClass} />
                  ...
                  {/* Message */}
                  <div className="relative group">
                    <label htmlFor="cp-message" className="block text-xs font-bold tracking-widest uppercase text-white/50 mb-2 group-focus-within:text-finova-cyan transition-colors">Message</label>
                    <div className="relative">
                      <textarea id="cp-message" name="message" placeholder="Tell us about your project..." value={formData.message} onChange={handleChange} onFocus={() => setFocusedField("message")} onBlur={() => setFocusedField(null)} required disabled={status.loading} className={`${inputClass} min-h-[160px] resize-none`} />
                ...
                <AnimatePresence mode="wait">
                  {status.success && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-3 rounded-xl bg-green-500/10 border border-green-500/30 px-6 py-4 text-sm text-green-400 font-mono overflow-hidden">
                      <CheckCircle className="h-5 w-5 shrink-0" />
                      {status.message}
                    </motion.div>
                  )}
                  {status.error && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-3 rounded-xl bg-red-500/10 border border-red-500/30 px-6 py-4 text-sm text-red-400 font-mono overflow-hidden">
